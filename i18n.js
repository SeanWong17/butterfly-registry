(function initWingI18n(globalObject, factory) {
  ...
  const UI = {
    zh: {
      ...
      imageOriginTitle: "图像来源与反馈",
      imageOriginText: "图像资料来自项目搜集与公开网络参考，并使用绘图模型进行统一化和视觉优化。图片仅用于导览，不替代标本、检索表、科研鉴定或执法依据。欢迎指出名称、分布、保护等级、分类或图像错误，也欢迎提供可核验的参考图片及其原始链接。",
    ...
    en: {
      ...
      imageOriginTitle: "Image sources and feedback",
      imageOriginText: "Images were assembled from project research and publicly available online references, then standardized and visually refined with drawing models. They are provided for orientation only and do not replace specimens, identification keys, scientific determinations, or enforcement evidence. Corrections to names, ranges, protection status, taxonomy, or imagery are welcome, as are verifiable reference images with their original links.",
